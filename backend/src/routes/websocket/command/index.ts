...
const commandRoutes: FastifyPluginAsync = async (fastify): Promise<void> => {
  ...
  fastify.post('/', {
    ...
    try {
      fastify.log.debug(`Command request body: ${JSON.stringify(request.body)}`);
      ...
      if (deviceConnection) {
        ...
        return reply.send({ status: 'success', message: 'Command sent to device' });
      } else {
        // Device is not currently connected
        return reply.send({ 
          status: 'pending', 
          message: 'Device is not currently connected. Command will be sent when device connects.'
        });
      }
    } catch (error) {
      fastify.log.error(`Error processing command: ${error instanceof Error ? error.message : 'Unknown error'}`);
