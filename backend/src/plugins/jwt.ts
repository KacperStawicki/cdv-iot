  ...
  fastify.decorate(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      try {
        ...
        if (device.user.email !== decoded.email) {
        ...
        return reply.status(500).send({ message: 'Error verifying device ownership' });
      }
    }
  );
