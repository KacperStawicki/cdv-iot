...
const wsRoutes: FastifyPluginAsync = async (fastify): Promise<void> => {
  ...
  async function ensureDeviceExists(deviceId: string) {
    try {
      ...
      if (!device) {
        ...
        if (!firstUser) {
          fastify.log.error("Cannot create device - no users exist in the database");
          return null;
        }
  ...
  fastify.get<{ Querystring: DeviceQueryParams }>('', { websocket: true }, (connection, req) => {
    ...
    if (deviceId) {
      ...
      connection.send(JSON.stringify({ type: 'welcome', message: `Connected as device ${deviceId}` }));
      
      // Ensure the device exists and send its configuration
      ensureDeviceExists(deviceId).then(device => {
        if (device) {
